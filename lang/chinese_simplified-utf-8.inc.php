<?php
/* $Id: chinese_simplified-utf-8.inc.php 12377 2009-04-19 12:01:48Z lem9 $ */
...
$strKeyname = '键名';
$strKill = '杀死';
$strKnownExternalBug = '%s 功能受到一个已知的 bug 影响，参见 %s';
$strKorean = '朝鲜语';
...
$strSQLParserBugMessage = '您可能发现了 SQL 解析器的 bug。请仔细检查您的查询，包括引号是否正确及是否匹配。其它可能的失败原因可能由于您上传了超过引用文本区域外的二进制数据。您还可以在 MySQL 命令行界面试一下您的查询。如果可能的话，以下会列出 MySQL 服务器的错误输出，这可能对您解决问题有一定的帮助。如果您仍然有问题，或者命令行界面执行成功而解析器出错，请将您的 SQL 查询缩减到导致问题的某一条语句，然后和下面剪切区中的数据一起提交一个 bug 报告：';
...
$strServers = '服务器';
$strSessionGCWarning = '您的PHP配置参数 [a@http://php.net/manual/en/session.configuration.php#ini.session.gc-maxlifetime@]session.gc_maxlifetime[/a] 短于您在 phpMyAdmin 中设置的 Cookies 有效期，因此您的登录会话有效期将会比您在 phpMyAdmin 中设置的时间要更短。';
...
$strSetupBZipDump_desc = '允许在导入和导出时使用 [a@http://en.wikipedia.org/wiki/Bzip2]bzip2[/a] 压缩';
...
$strSetupGZipDump_desc = '允许在导入和导出时使用 [a@http://en.wikipedia.org/wiki/Gzip]gzip[/a] 压缩';
...
$strSetupServers_DisableIS_desc = '参见 [a@http://sf.net/support/tracker.php?aid=1849494]PMA bug 跟踪系统 (外链，英文)[/a] 和 [a@http://bugs.mysql.com/19588]MySQL Bugs (外链，英文)[/a]';
...
$strSetupServers_SignonSession_desc = '参见[a@http://wiki.phpmyadmin.net/pma/auth_types#signon]认证方式[/a]中的例子';
$strSetupServers_SignonSession_name = 'Signon 会话名';
$strSetupServers_SignonURL_name = '登录地址';
$strSetupServers_auth_swekey_config_desc = '[a@http://swekey.com]SweKey 硬件认证[/a]的配置文件路径 (请勿置于你的文档根文件夹，推荐使用：/etc/swekey.conf)';
...
$strSetupWarning = '警告';
$strSetupZipDumpExportWarning = '此系统目前不支持 [a@?page=form&amp;formset=features#tab_Import_export]Zip 压缩[/a]所必须的 (%s) 函数。';
$strSetupZipDumpImportWarning = '此系统目前不支持 [a@?page=form&amp;formset=features#tab_Import_export]Zip 解压缩[/a]所必须的 (%s) 函数。';
$strSetupZipDump_desc = '允许在导入和导出时使用 [a@http://en.wikipedia.org/wiki/ZIP_(file_format)]ZIP[/a] 压缩';
...
$strShowStatusSelect_full_joinDescr = '没有使用索引的多表查询数。如果该值不为0，您应该检查是否已经为表建立了适当的索引。';
...
$strShowStatusSelect_range_checkDescr = '没有使用索引但在每行之后检查索引使用的多表查询数。如果该值不为0，您应该检查是否已经为表建立了适当的索引';
